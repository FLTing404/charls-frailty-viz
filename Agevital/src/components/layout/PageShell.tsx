...
export function PageShell({ children, className, title, actions }: PageShellProps) {
  return (
    <div className={cn('relative flex h-full w-full flex-col overflow-hidden', className)}>
      ...
      {(title || actions) && (
        <div className="relative z-10 flex shrink-0 items-center justify-between gap-2 border-b border-ink/10 bg-paper-alt/50 px-1.5 lg:px-2 h-7 lg:h-9 backdrop-blur-[1px]">
          <div className="min-w-0">{title}</div>
          {actions && <div className="shrink-0">{actions}</div>}
        </div>
      )}
      <div className="relative z-10 min-h-0 flex-1 overflow-hidden px-1 pb-1 pt-0">{children}</div>
    </div>
  )
}
