...
export function ParallelCoordinatesChart({
  ...
  // Drop a dimension if > 70% of values are 0 or NaN.
      ...
      series: [
        {
          ...
          // Let ECharts handle brush visibility via inactiveOpacity=0.
          // We always feed ALL data; ECharts hides lines that don't match
          // the multi-axis AND brush selection.
          inactiveOpacity: 0,
          activeOpacity: 1,
          data: parallelData.map((d) => ({
            ...
            lineStyle: {
              color: frailtyLineColor(d.frailty_cat),
            },
          })),
        },
      ],
    }
  }, [parallelData, focusDimension, ranges, visibleDims])

  // Map of parallelData index → record id for looking up brush-matched records.
  const idByIndexRef = useRef<string[]>([])
  idByIndexRef.current = parallelData.map((d) => d.id)
  ...
  const handleAxisAreaSelected = useCallback(
    (event: { batch?: { parallelAxisIndex: number; intervals: [number, number][] }[] }) => {
      ...
      // AND logic: must fall within selected intervals on EVERY brushed axis.
  ...
  // Full rebuild: notMerge:true replaces everything including data.
  // inactiveOpacity:0 makes ECharts hide non-matching lines natively,
  // so we always feed the full parallelData — no displayData filtering needed.
  useEffect(() => {
