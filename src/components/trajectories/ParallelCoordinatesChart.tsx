...
const AXIS_ORDER = ['ace', 'sleep', 'social', 'depression', 'fi'] as const
const AXIS_DIMS = ['ace', 'sleep', 'social', 'depression', 'fi']
...
export function ParallelCoordinatesChart({
  ...
  const parallelData = useMemo(
    () =>
      records.map((r) => ({
        value: [r.ace, r.sleep, r.social, r.depression, r.fi],
        id: r.id,
  ...
  const handleAxisAreaSelected = useCallback(
    (event: { batch?: { parallelAxisIndex: number; intervals: [number, number][] }[] }) => {
      ...
      const filtered = recordsRef.current.filter((r) => {
        const vals = [r.ace, r.sleep, r.social, r.depression, r.fi]
